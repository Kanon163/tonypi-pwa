...
当前无法在本机替代安卓 Chrome 与真实 BCI 头环完成实测。探针已覆盖设备选择、GATT 连接、服务/特征值枚举、read/notify 尝试、断连记录和 `BciSample` 候选映射。

## 使用方式

1. 通过 HTTPS 或 localhost 打开 `web-bluetooth-probe.html`。
2. 使用安卓 Chrome。
3. 若知道 BCI 服务 UUID，填入“可选服务 UUID”。
4. 点击“选择并连接 BCI”。
...
- 未知协议时使用 `acceptAllDevices` 触发设备选择。
- 可选服务 UUID 由测试者输入，避免提前固化厂商协议。
- 连接后枚举 primary services 和 characteristics。
- 对可读特征值尝试 `readValue()`。
